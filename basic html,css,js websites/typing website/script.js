...
const paragraphs = [
    "Authors often misinterpret the lettuce as a folklore rabbi, when in actuality it feels more like an uncursed bacon. Pursued distances show us how mother-in-laws can be charleses. Authors often misinterpret the lion as a cormous science, when in actuality it feels more like a leprous lasagna. Recent controversy aside, their band was, in this moment, a racemed suit. The clutch of a joke becomes a togaed chair. The first pickled chess is. In modern times the first scrawny kitten is, in its own way, an input. An ostrich is the beginner of a roast. An appressed exhaust is a gun of the mind. A recorder is a grade from the right perspective. A hygienic is the cowbell of a skin. Few can name a dun brazil that isn't a highbrow playroom. The unwished beast comes from a thorny oxygen. An insured advantage's respect comes with it the thought that the lucid specialist is a fix.",
...
let spanWords=[];
let wordsIndex=0;
let givenTextWords=document.getElementById("givenTextWords");
let wordsSpans=[];
let countDown=document.getElementById("countDown");
let input=document.getElementById("input");
let popupBox=document.getElementById("popupBox");
let wpmCount=document.getElementById("wpmCount")
let cpmCount=document.getElementById("cpmCount")
let wrongsCount=document.getElementById("wrongsCount")
let unattemptedCount=document.getElementById("unattemptedCount")
let wrongWords=[];
let correctWords=[];
let unttemptedWords=[];
let correctChars="";
...
const generateRandomWords=()=>{    
    paraIndex=parseInt(Math.random()*(paragraphs.length-1))
    words=paragraphs[paraIndex].split(" ");
}
const renderWords=()=>{
    givenTextWords.innerHTML="";
    words.forEach((eachWord,index)=>{
       givenTextWords.insertAdjacentHTML("beforeend",`<span id=${"span"+index}>${eachWord}</span>`);
    })
    wordsSpans=givenTextWords.children;
}
const clearInput=()=>{
    input.value=null;
}
const reInitiateWordsIndex=()=>{
    wordsIndex=0;
}
const restartTyping=()=>{
    givenTextWords.style.top="0px";
    time=maxTime;
    clearInput();
    isStartTimer=false;
    generateRandomWords();
    renderWords();
    reInitiateWordsIndex();
    correctWords=[];
    wrongWords=[];
    unttemptedWords=[];
}
const disableInput=(isDisabled)=>{
    ...
    if(isDisabled){
        input.disabled=true; 
        input.style.background="rgb(126 163 198 / 50%)";
    }
    else{
...
const showPopupBox=()=>{
    if(popupBox.classList.contains("hidePopup")){
...
const showResults=()=>{
    Array.from(wordsSpans).forEach((eachWordSpan)=>{
         let isContainsWrong=eachWordSpan.classList.contains("wrong");
         let isContainsCorrect=eachWordSpan.classList.contains("correct");
         if(isContainsCorrect){
             correctWords.push(eachWordSpan.innerText);
         }
         else if(isContainsWrong){
             wrongWords.push(eachWordSpan.innerText);
         }
         else if(!(isContainsWrong && isContainsCorrect)){
             unttemptedWords.push(eachWordSpan.innerText);
         }
    })
    for(let i=0;i<correctWords.length;i++){
        correctChars=correctChars+correctWords[i];
    }
    cpmCount.innerText=correctChars.length;
    wpmCount.innerText=correctWords.length;
    wrongsCount.innerText=wrongWords.length;
    unattemptedCount.innerText=unttemptedWords.length;
    showPopupBox();
}
const matchWords=(event)=>{
    isStartTimer=true;
    const typedText=event.target.value;
    const typedTextLen=typedText.length;
    const lastChar=typedText[typedTextLen-1];
    if(lastChar==" "){
        event.target.value=null;
        if(typedTextLen>1 && typedText[typedTextLen-2]!=" "){
            let inputWord=typedText.substring(0,typedTextLen-1);
            let spanElement=wordsSpans[wordsIndex];
            if(spanElement.innerText===inputWord){
                document.getElementById(`${spanElement.id}`).classList.add("correct");
            }
            else{
                document.getElementById(`${spanElement.id}`).classList.add("wrong")
            }
            let topVal=parseInt(givenTextWords.style.top.substring(0,givenTextWords.style.top.length-2));
            if(wordsSpans[wordsIndex+1] && document.getElementById(`${wordsSpans[wordsIndex+1].id}`).getBoundingClientRect().top>document.getElementById(`${spanElement.id}`).getBoundingClientRect().top){
                givenTextWords.style.top=`${topVal-62}px`
            }
